...
var paletteHideByType = function (e){
  ...
  alert(myType)
  if (acg_Nodes.includes(myType)){
    filter="node[type ='"+myType +"']";  
    }
  else{
   
    if (acg_Node_Specializations.includes(myType)){
      alert("Ok")
      filter="node[specialization ='"+myType +"']";  
      }
    else{
      myRelationshipType=myType+"-relationship"
      if (acg_Edges.includes(myRelationshipType)){
        filter="edge[edgeType='"+myType +"']";  
        }
      else{  
        if (acg_Edge_Specializations.includes(myRelationshipType)){
                      filter="edge[specialization ='"+myType +"']";  
          } 
        }
      } 
    }
...
//          <input id="acgLayout" style="color:blue;" onchange="acgLayout=this.value;cyLayout.name=acgLayout"; value="fcose" size="25" ></div>` },
          ...
          { type: 'new-line' },
          { type: 'check',  id: 'nodes', text: 'nodes', checked: true, tooltip: 'Consider nodes for expression filtering' },
